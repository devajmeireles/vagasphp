<div x-data="{ format : @entangle('format'), remuneration : @entangle('job.remuneration.type') }">
    <div class="mb-4">
        ...
        <div class="mb-2">
            <div class="grid grid-cols-6 gap-2">
                <div class="col-span-full">
                    <x-input.select x-model="format" @change="$wire.call('restart')" label="Formato">
                        <option value="">@lang('- Selecione')</option>
                        <option value="redirectable">@lang('Redirecionável')</option>
                        <option value="detailable">@lang('Detalhável')</option>
                    </x-input.select>
                    <div class="mt-2" x-show="format === 'redirectable'" x-transition x-cloak>
                        <x-alert blue>
                            @lang('O click irá redirecionar o candidato para uma URL. <b>Ideal para casos onde você utiliza um outro sistema de anúncio de vagas.</b>')
                        </x-alert>
                    </div>
                </div>
                <div class="col-span-full" x-show="format === 'redirectable'" x-transition x-cloak>
                    <x-input label="Titulo" wire:model.defer="job.title"/>
                </div>
                <div class="col-span-3" x-show="format === 'redirectable'" x-transition x-cloak>
                ...
                <div class="col-span-full" x-show="format === 'redirectable'" x-transition x-cloak>
                    <x-input label="URL de Redirecionamento" wire:model.defer="job.link"/>
                </div>
            </div>
        </div>
        <div class="grid grid-cols-6 gap-2" x-show="format === 'detailable'" x-transition x-cloak>
            <div class="col-span-full">
                <x-input label="Titulo" wire:model.defer="job.title"/>
            </div>
            <div class="col-span-full">
                ...
                <div class="mt-2 ml-2">
                    <p class="text-xs text-gray-700">@lang('A descrição aceita formatações em markdown.')</p>
                </div>
            ...
            <div class="col-span-full" x-transition>
                ...
                <div class="mt-2">
                    <x-alert blue>
                        @lang('Quando uma candidatura ocorrer você será notificado através de um <b>webhook para um canal do Slack ou via e-mail.</b>')
                    </x-alert>
                </div>
            </div>
        </div>
        <div class="mt-4 flex justify-end">
            <x-button ::disabled="format === null || format === ''"
                      wire:click="create"
